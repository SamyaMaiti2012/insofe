...
Fist clearing the enviromment for the project 
```{r}
rm(list = ls(all=TRUE))
```
# Loading and understanding the healthCaseData 
loading the path and the healthCaseDataset 
```{r}
getwd() #shows my working directry 
...
```
# exploring the healthCaseData set
```{r}
dim(healthCaseData)
head(healthCaseData)
...
head(validation_data_Imputed_stand)
