...
```{r}
getwd()
setwd("/Users/samyam/Documents/Samya/Insofe/insofe/day12/20190106_Batch56_CSE7302c_Lab03_LogisticRegressionActivity")
getwd()
...
```{r}
train_row = createDataPartition(bank_data$y, p=0.7, list = F)

train_bank = bank_data[ train_row, ]
test_bank = bank_data[ -train_row, ]
...
model_bank = glm(formula = y ~ ., data = (train_bank), family = binomial)
```
...
prob_train = predict(model_bank, type = "response")
prob_train[1:6]
```
...
prob_test = predict(model_bank, test_bank, type = "response")
prob_test[1:6]

pred_test = ifelse(prob_test > 0.1, 1, 0)
pred_train = ifelse(prob_train > 0.1, 1, 0)
...
```{r}
#confusionMatrix(pred_train, test_bank$y)
table(test_bank$y,pred_test)
table(train_bank$y,pred_train)
